...
int get_file(char str[])
{
    ...
    if(str[0]=='"')
    {
        if(str_with_double_quotation(str)==ERRORERROR)
        {
            return ERRORERROR;
        }
    }else if(str[0]=='\n')
    ...
    for(int i=0;i<3;i++)
    {
        ...
        if(strcmp(flag,"--str")==0)
        {
            if(str_check!=0)
                return error(*invalid_input);
            str_check++;
            if(get_str(input_str)==ERRORERROR)
                return error(*invalid_input);
            continue;
        }
        if(strcmp(flag,"--pos")==0)
        {
            ...
            if(pos_check!=0)
                return error(*invalid_input);
            pos_check++;
            if((position=find_position(line,pos,path))==ERRORERROR)
...
int remove_str()
{
    ...
    if((position=check_position(path,sign,size,position))==ERRORERROR)
...
int search(char *str1,char *str2,int pos)
{
    ...
    for(int i=limit-1;i>=0;i--)
    {
        ...
        {
            if((tah=search(&str1[strlen(str1)+1],&str2[i+strlen(str1)],pos+i+strlen(str1)))!=ERRORERROR)
                return tah;
        }
    }
    return ERRORERROR;
...
int get_str_for_find(char str[])
{
    ...
    if(str[0]=='"')
    {
        if(str_for_find_with_double_quotation(str)==ERRORERROR)
            return ERRORERROR;
        return OK;
    }else
    {
...
int start_search(char input_str[],char file[],int mode,int at)
{
    ...
    if(input_str[0]=='\0')
    {
        if((tah=search(&input_str[1],&file[0],0))!=ERRORERROR)
        {
            sar=1;
            tedad++;
            i=tah;
            if(different_modes(mode,sar,tedad,file,at)==ERRORERROR)
                return OK;
        }
        for(;i<strlen(file);i++)
        {
            if(file[i]=='\n' || file[i]==' ')
            {
                if((tah=search(&input_str[1],&file[i+1],i+1))!=ERRORERROR)
                {
                    sar=i+2;
                    tedad++;
                    i=tah;
    ...
    for(;i<limit;i++)
    {
        ...
        //for(int j=0;j<strlen(input_str);j++)
            //printf("%c",file[i+j]);
            if((tah=search(&input_str[strlen(input_str)+1],&file[i+strlen(input_str)],i+1+strlen(input_str)))!=ERRORERROR)
            {
                sar=i+1;
                tedad++;
                i=tah;
...
int find()
{
    ...
    for(int i=0;i<2;i++)
    {
        ...
        if(strcmp(flag,"--str")==0)
        {
            if(str_check!=0)
                return error(*invalid_input);
            str_check++;
            if(get_str_for_find(input_str)==ERRORERROR)
                return error(*invalid_input);
            continue;
        }
    }
    ////
    int mode;
    if(get_option(&at,&count,&byword,&all)==ERRORERROR)
       return error(*invalid_input);
    if(count==1)
    {
...
int start_search_for_replace(char input_str[],char file[],int mode,int at,char replace_str[],char path[])
{
    ...
    if(input_str[0]=='\0')
    {
        if((tah=search(&input_str[1],&file[0],0))!=ERRORERROR)
        {
            ...
            tedad++;
            i=tah;
            if(different_modes_for_replace(mode,sar,path,at,replace_str,tedad,tah)==ERRORERROR)
                return OK;
        }
        for(;i<strlen(file);i++)
        {
...
int replace()
{
    ...
    for(int i=0;i<3;i++)
    {
        ...
        if(strcmp(flag,"--str2")==0)
        {
            if(str2_check!=0)
                return error(*invalid_input);
            str2_check++;
            if(get_str_for_find(replac_str)==ERRORERROR)
                return error(*invalid_input);
            continue;
        }
    }
    ////////////
    int mode;
    if(get_option_for_replace(&at,&all)==ERRORERROR)
       return error(*invalid_input);
    if(all==0&&at==-10)
    {
        mode=0;
...
{
    if(get_file_for_grep(str)==ERRORERROR)
        return ERRORERROR;
    if(str[0]=='/')
        strcpy(str,&str[1]);
    if(str[strlen(str)-1]=='/')
    ...
    for(int i=0;i<4;i++)
    {
        ...
        if(strcmp(flag,"--files")==0)
        {
            ...
            for(int i=0;;i++)
            {
                ...
                if(file_input_for_grep(path[i])==ERRORERROR)
                {
                    //printf("#");
                    if(path[i][0]=='\n')
                    {
                ...
                //printf("%s\n",path[i]);
                if(check_wrong_address(path[i])==ERRORERROR)
                    return error(*invalid_address);
                if(check_existance_of_file(path[i])==ERRORERROR)
                    return error(*no_such_file);
            }
            break;
        }
...
int undo()
{
    ...
    {
        return error(*no_such_file);
        /*if(check_existance_of_file(path2)==ERRORERROR)
            return error(*no_such_file);
        go_to_address(path);
        FILE*fptr=fopen(path,"w");
        fclose(fptr);*/
